...
/**
 * 记录了ResultSet中的一些元数据，并且提供了一系列操作，ResultSet的辅助方法
 *
 * @author Iwao AVE!
 */
public class ResultSetWrapper {
  ...
  //记录了未映射的列名，其中key是ResuultMap对象的id,value是该ResultMap对象未映射的列名集合
  ...
  private void loadMappedAndUnmappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
    //分别记录ResultMap中映射的列名和未映射的列名
    List<String> mappedColumnNames = new ArrayList<>();
    List<String> unmappedColumnNames = new ArrayList<>();
    final String upperColumnPrefix = columnPrefix == null ? null : columnPrefix.toUpperCase(Locale.ENGLISH);
    //ResultMap中定义的列名加上前缀，得到实际的类名
    ...
    //将ResultMap的id和列前缀组成key,将Resultmap映射的类名及未映射的列名保存到mappedColumnNamesMap和unMappedColumnNamesMap中
