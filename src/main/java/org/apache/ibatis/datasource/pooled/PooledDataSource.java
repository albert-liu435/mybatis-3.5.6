...
public class PooledDataSource implements DataSource {
  ...
  //真正的数据库连接对象由UnpooledDataSource对象创建，用于生成真实的数据库连接对象
    ...
    while (conn == null) {
      ...
      synchronized (state) {
        ...
        if (conn != null) {
          ...
          //连接是否有效
          if (conn.isValid()) {
            //添加到activeConnections集合并进行相应的统计操作
            if (!conn.getRealConnection().getAutoCommit()) {
              conn.getRealConnection().rollback();
            }
